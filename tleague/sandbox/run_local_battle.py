...
def main(_):
    ...
    if FLAGS.policy_config:
        ...
        policy_config = {}
    agents = [
        PGAgent(policy, ob_sp, ac_sp, n_v=FLAGS.n_v, scope_name=name, policy_config=policy_config)
        for policy, ob_sp, ac_sp, name in zip(policies, env.observation_space.spaces, env.action_space.spaces, ['p1', 'p2'])
    ]
