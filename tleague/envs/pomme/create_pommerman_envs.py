...
def create_pommerman_env(arena_id, env_config=None, inter_config=None):
    ...
    if arena_id in ['pommerman_v1', 'pommerman_v2']:
        env = pommerman_2v2(
            'PommeTeam-v0', random_side=random_side, agent_list=agent_list, rule_agents=rule_agents, replay_dir=replay_dir)
...
def pommerman_env_space(arena_id, env_config=None, inter_config=None):
    env = create_pommerman_env(arena_id, env_config=env_config, inter_config=inter_config)
    env.reset()
