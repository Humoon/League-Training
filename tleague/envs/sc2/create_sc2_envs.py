...
class OppoObsComponentsV2:
    ...
    def __init__(self, game_version, dict_space, max_bo_count, max_bobt_count, zmaker_version, **kwargs):
        from timitate.lib6.pb2feature_converter import GlobalFeatMaker,  \
          ImmZStatMaker
        self._dict_space = dict_space
        self._components = [
            GlobalFeatMaker(game_version),
            ImmZStatMaker(max_bo_count=max_bo_count, max_bobt_count=max_bobt_count, zstat_version=zmaker_version)
        ]
    ...
    def make(self, pb):
        if self._dict_space:
            ...
            for com in self._components:
                dict_features.update(OrderedDict(['OPPO_' + k, v] for k, v in zip(com.tensor_names, com.make(pb))))
...
class StatZStatDist(Wrapper):
    ...
    def __init__(self, env):
        ...
        self.SeqLevDist = {}
        self.info = {}
        self.required_keys = ['Z_BUILD_ORDER', 'Z_BUILD_ORDER_COORD', 'IMM_BUILD_ORDER', 'IMM_BUILD_ORDER_COORD']
                ...
                if isinstance(ob, dict) and set(self.required_keys) <= set(ob):
                    if np.sum(ob['Z_BUILD_ORDER']) > 0:
                        if ind not in self.SeqLevDist:
                            self.SeqLevDist[ind] = SeqLevDist_with_Coord(ob['Z_BUILD_ORDER'], ob['Z_BUILD_ORDER_COORD'])
                        self.info[key] = self.SeqLevDist[ind].lev_dist(ob['IMM_BUILD_ORDER'], ob['IMM_BUILD_ORDER_COORD'])
                else:
...
def make_sc2_base_env(n_players=2,
                      step_mul=8,
                      version='4.7.0',
...
def make_sc2full_v8_interface(zstat_data_src='',
    ...
    inter = FullObsIntV7(
        ...
        **kwargs)
    inter = FullActIntV6(inter, max_noop_num=len(noop_nums), map_resolution=output_map_size, dict_space=dict_space, **kwargs)
...
def create_sc2full_formal_env(inter_fun,
    ...
    if centralized_value:
        ...
        if inter_fun in [make_sc2full_v8_interface]:
            ...
            env = SC2ZergOppoObsAsObs(env, dict_space,
                                      OppoObsComponentsV2(version, dict_space, max_bo_count, max_bobt_count, zmaker_version))
        else:
...
def create_sc2_env(arena_id, env_config=None, inter_config=None):
    ...
    map_name = ('KairosJunction' if 'map_name' not in env_config else env_config['map_name'])
    use_trt = False if 'use_trt' not in env_config else env_config['use_trt']
    astar_rwd_version = ('v3' if 'astar_rwd_version' not in env_config else env_config['astar_rwd_version'])
    compensate_win = (True if 'compensate_win' not in env_config else env_config['compensate_win'])
    ...
    game_core_config = ({} if 'game_core_config' not in env_config else env_config['game_core_config'])
    early_term = (True if 'early_term' not in env_config else env_config['early_term']) # enabling by default
...
def sc2_env_space(arena_id, env_config=None, inter_config=None):
    ...
    import distutils
    version = '4.10.0'
    version_ge_414 = (distutils.version.LooseVersion(version) >= distutils.version.LooseVersion('4.1.4'))
    if arena_id in ['sc2full_formal8_dict']:
        ...
        if centralized_value:
            ...
            ob_space = spaces.Dict(OrderedDict(list(ob_space.spaces.items()) + list(com.space.spaces.items())))
    else:
