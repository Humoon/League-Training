...
class DataServer(object):
    ...
    def __init__(self,
        ...
        for i in range(num_dataset):
            if self.version == 'v2':
                dataset = tf.data.Dataset.range(batch_worker_num).apply(
                    tf.contrib.data.parallel_interleave(
                        lambda x: tf.data.Dataset.from_generator(self._data_generator, self.dtypes, self.shapes, args=(x,)),
            ...
            if gpu_num <= 0:
                ...
                prefetch_op = tf.contrib.data.prefetch_to_device(device="/gpu:" + str(gpu_id), buffer_size=prefetch_buffer_size)
                dataset = dataset.apply(prefetch_op)
            iterator = dataset.make_one_shot_iterator()
            self.input_datas.append(ds.make_structure(iterator.get_next()))

    def _mk_rm(self, rm_size, unroll_length, batch_size, rollout_length, version, decode):
        if version == 'v2':
            self._replay_mem = ReplayMemMP(
                rm_size, unroll_length, batch_size, rollout_length, decode=decode, n_process=self._batch_worker_num)
        else:
            self._replay_mem = ReplayMem(rm_size, unroll_length, batch_size, rollout_length, decode=decode)
    ...
    def ready_for_train(self):
        if not self._replay_mem.ready_for_sample():
            logger.log('train data queue not full ({}/{} unrolls, wait...)'.format(
    ...
    def _pull_data(self):
        while True:
            ...
            if data[0] == self._model_id or is_inherit(data[0], self._model_id):
                ...
                logger.log('Receive data from model ' + data[0] + ', while current model is ' + self._model_id)

    def _print_infos(self):
        # filter the zstat infos, can not average
        filter = lambda d: dict([(_k, _v) for _k, _v in d.items() if _v is not None and not isinstance(_v, str)])
        stat = Counter({})
...
class ImDataServer(object):
    ...
    def __init__(
        ...
        self._pull_data_threads = [Thread(target=self._pull_data, daemon=True) for _ in range(pull_worker_num)]
        self._train_rm = ImpSampReplayMem(rm_size, unroll_length, batch_size, rollout_length, min_train_sample_num)
        self._val_rm = ImpSampReplayMem(rm_size, unroll_length, batch_size, rollout_length, min_val_sample_num)
        ...
        train_dataset = tf.data.Dataset.range(train_generator_worker_num).apply(
            tf.contrib.data.parallel_interleave(
                lambda x: tf.data.Dataset.from_generator(self._create_data_generator(self._train_rm), dtypes, shapes).apply(
        ...
        if use_gpu:
            prefetch_op = tf.contrib.data.prefetch_to_device(device="/gpu:0", buffer_size=train_generator_worker_num)
        ...
        val_dataset = tf.data.Dataset.range(val_generator_worker_num).apply(
            tf.contrib.data.parallel_interleave(
                lambda x: tf.data.Dataset.from_generator(self._create_data_generator(self._val_rm), dtypes, shapes).apply(
                    tf.contrib.data.batch_and_drop_remainder(batch_size)),
    ...
    def ready_for_val(self):
        if not self._val_rm.ready_for_sample():
            logger.log('val data queue not full ({}/{} unrolls, wait...)'.format(
    ...
    def _pull_data(self):
        while True:
            ...
            if not isinstance(replay_task, ImitationTask):
                raise RuntimeError("received task %s not recognized".format(replay_task))
