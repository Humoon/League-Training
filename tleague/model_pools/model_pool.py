...
class ModelPool(object):
    ...
    def _write_worker(self):
        ...
        if msg == 'model':
            ...
            logger.log(now() + 'on msg write model.', 'key: {},'.format(model.key), 'create time: {}'.format(model.createtime))
    ...
    def _read_worker(self):
        ...
        if msg == 'model': # get one Model
            ...
            self._rep_socket.send_pyobj(
                ModelPoolErroMsg('Key {} not exits'.format(key)) if key not in self._model_pool else self._model_pool[key])
            ...
            self._rep_socket.send_pyobj({
                k: (ModelPoolErroMsg('Attribute not exits') if not hasattr(v, attr) else getattr(v, attr))
            ...
            key = self._rep_socket.recv_string()
            self._rep_socket.send_pyobj(
                ModelPoolErroMsg('Key not exits') if key not in self._model_pool else None if key not in
                self._learner_meta else self._learner_meta[key])
            ...
            key = self._rep_socket.recv_string()
            self._rep_socket.send_pyobj(
                ModelPoolErroMsg('Key not exits') if key not in self._model_pool else (
                    ModelPoolErroMsg('Attribute not exits'
                                    ) if not hasattr(self._model_pool[key], attr) else getattr(self._model_pool[key], attr)))
