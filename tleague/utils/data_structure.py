...
class PGData(DataStructure):
    ...
    def __init__(self,
        ...
        _fields = ['X', 'A']
        shape_dtype = lambda x: (x.shape, x.dtype)
        specs = [map_gym_space_to_structure(shape_dtype, ob_space), map_gym_space_to_structure(shape_dtype, ac_space)]
        templates = [template_structure_from_gym_space(ob_space), template_structure_from_gym_space(ac_space)]
        if random_policy:
            _fields.append('neglogp')
            specs.append(map_gym_space_to_structure(lambda x: ([], np.float32), ac_space))
        ...
        if distillation:
            ...
            param_shape_dtype = map_gym_space_to_structure(logit_shape_dtype, ac_space)
...
class PPOData(PGData):
    ...
    def __init__(self, ob_space, ac_space, n_v, use_lstm=False, hs_len=None, distillation=False, use_oppo_obs=False):
        super(PPOData, self).__init__(ob_space, ac_space, n_v, use_lstm, hs_len, distillation, use_oppo_obs)
...
class VtraceData(PGData):
    ...
    def __init__(self, ob_space, ac_space, n_v, use_lstm=False, hs_len=None, distillation=False, use_oppo_obs=False):
        super(VtraceData, self).__init__(ob_space, ac_space, n_v, use_lstm, hs_len, distillation, use_oppo_obs)
...
class DDPGData(PGData):
    ...
    def __init__(self, ob_space, ac_space, n_v, use_lstm=False, hs_len=None, distillation=False, use_oppo_obs=False):
        super(DDPGData, self).__init__(
            ob_space, ac_space, n_v, use_lstm, hs_len, distillation, use_oppo_obs, random_policy=False)
        self.fields.extend(['r', 'discount'])
        self.specs.extend([([n_v], np.float32), ([], np.float32)])
        self.templates.extend([None, None])
        super(PGData, self).__init__(self.fields, self.specs, self.templates)
...
class InfData(DataStructure):
    ...
    def __init__(self, ob_space, ac_space, use_self_fed_heads=True, use_lstm=False, hs_len=None):
        shape_dtype = lambda x: (x.shape, x.dtype)
        _fields = ['X']
