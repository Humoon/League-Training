...
class DDPGLearner(PGLearner):
    ...
    def _build_ops(self):
        ...
        init_updates, soft_updates = self.get_target_updates(self.params, self.target_params, self.tau)
        _, vf_soft_updates = self.get_target_updates(self.params_vf, self.target_params_vf, self.tau)
        ...
        def train_batch(lr, cliprange, lam, weights=None):
            ...
            return self.sess.run(
                self.losses + [self.clip_grad_norm, self.nonclip_grad_norm, self.param_norm, self._train_batch, soft_updates],
        ...
        def burn_in(lr, cliprange, lam, weights=None):
            ...
            return self.sess.run(
                self.losses +
                [self.clip_grad_norm_vf, self.nonclip_grad_norm_vf, self.param_norm, self._burn_in, vf_soft_updates],
                feed_dict=td_map)[0:len(self.loss_names)]
    ...
    def _build_train_op(self):
        grads_and_vars = self.trainer.compute_gradients(self.loss, [p for p in self.params if p not in self.params_vf])
        grads_and_vars += self.trainer.compute_gradients(self.vf_loss, self.params_vf)
