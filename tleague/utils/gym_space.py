...
if __name__ == '__main__':
    space = Tuple([
        ...
        Dict([('x', Discrete(6)), ('y', Box(0.0, 1.0, shape=(8, 32), dtype=np.float32))])
    ])
    inst = (np.ones(shape=(2, 3), dtype=np.float32), OrderedDict([('x', 5), ('y', np.zeros(shape=(8, 32), dtype=np.float32))]))
