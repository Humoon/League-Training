...
class MutableHyperparam(Hyperparam):
    ...
    def __str__(self):
        s_nn = _str_dict({'lr': self.learning_rate, 'cliprange': self.cliprange, 'lam': self.lam, 'gamma': self.gamma})
...
class MutableHyperparamRandMaxKOnes(Hyperparam):
  ...
  """
    ...
    def __init__(self, learning_rate=1e-4, cliprange=0.1, lam=0.9, gamma=0.99, reward_weights=[1.0] * 11, K=4, **kwargs):
        self.learning_rate = learning_rate
        self.cliprange = cliprange
        self.lam = lam
        self.gamma = gamma
...
class MutableHyperparamRandPerturb(Hyperparam):
  ...
  """
    ...
    def __init__(self,
        ...
        self.reward_weights = [_loguniform(init_weight_low, init_weight_high) * scale for scale in reward_scale]
    ...
    def mutate(self, **kwargs):
        if random.random() < self._perturb_prob:
            ...
            ratios = [
                np.random.uniform(1 - self._weight_perturb_percentage, 1 + self._weight_perturb_percentage)
    ...
    def mutate(self, **kwargs):
        if random.random() < self._perturb_prob:
            ...
            for ind in self._perturb_ind:
                ratio = np.random.uniform(1 - self._weight_perturb_percentage, 1 + self._weight_perturb_percentage)
...
class ConstantHyperparam(Hyperparam):
    ...
    def __init__(self, learning_rate=1e-4, cliprange=0.1, lam=0.9, gamma=0.99, reward_weights=None, sigma=1., **kwargs):
        self.learning_rate = learning_rate
        self.cliprange = cliprange
        self.lam = lam
        self.gamma = gamma
...
class DiscreteDistribHyperparam(ConstantHyperparam):
  ...
  """
    ...
    def __init__(self, learner_id=None, n_distrib=100, lrn_id_to_distrib_type=None, zeroing_prob=0.2, **kwargs):
        ...
        if dt == 'uniform':
            self.distrib = (np.ones(shape=(self.n_distrib,), dtype=np.float32) / self.n_distrib)
        elif dt == 'one_hot':
    ...
    def __str__(self):
        ...
        s_distrib = ('n_distrib: {}, learner_id: {}, distrib_type: {}, '
                     'zeroing_prob: {}').format(self.n_distrib, self.learner_id, self.lrn_id_to_distrib_type[self.learner_id],
                                                self.zeroing_prob)
        s_weight = 'rwd weight: ' + _str_list(self.reward_weights)
...
class DiscreteDistribHyperparamV2(ConstantHyperparam):
  ...
  """
    ...
    def __init__(self,
        ...
        self.lrn_id_to_n_distrib = lrn_id_to_n_distrib or {'lrngrp0': 8, 'lrngrp1': 8, 'lrngrp2': 8}
        ...
        self.lrn_id_to_zeroing_prob = lrn_id_to_zeroing_prob or {'lrngrp0': 0.2, 'lrngrp1': 0.0, 'lrngrp2': 0.0}
        ...
        if 'cur_idx_one_hot' not in blackboard.__dict__:
            print('DiscreteDistribHyperparamV2: Initialize' ' blackboard.cur_idx_one_hot only once')
            # a {lrn_id: count} dict for the distrib type 'cyclic_one_hot'
            blackboard.cur_idx_one_hot = {lrn_id: 0 for lrn_id in self.lrn_id_to_n_distrib}
        print('DiscreteDistribHyperparamV2: blackboard.cur_idx_one_hot={}'.format(blackboard.cur_idx_one_hot))
        ...
        if dt == 'uniform':
            ...
            assert self.lrn_id_to_max_n_cyclic[self.learner_id] is not None, ('None max_n_cyclic in {}'.format(self.learner_id))
            blackboard.cur_idx_one_hot[self.learner_id] = (
                blackboard.cur_idx_one_hot[self.learner_id] % self.lrn_id_to_max_n_cyclic[self.learner_id])
        else:
    ...
    def _mutate_reward_weights(self):
        ...
        return [
            ...
            for rw, prob in zip(self._reward_weights_orig, self.lrn_id_to_reward_weights_activate_prob[self.learner_id])
        ]
    ...
    def __str__(self):
        ...
        s_distrib = ('learner_id: {}, n_distrib: {}, distrib_type: {}, '
                     'distrib: {}, zeroing_prob: {}').format(self.learner_id, self.n_distrib,
                                                             self.lrn_id_to_distrib_type[self.learner_id], self.distrib,
                                                             self.zeroing_prob)
        s_weight = 'rwd_weights: ' + _str_list(self.reward_weights)
        s_weight_orig = 'rwd_weights_orig' + _str_list(self._reward_weights_orig)
        return type(self).__name__ + ': ' + ', '.join([s_nn, s_distrib, s_weight, s_weight_orig])
...
class DiscreteDistribHyperparamV3(ConstantHyperparam):
  ...
  """
    ...
    def _mutate_reward_weights(self):
        ...
        return [
            rw if random.random() < prob else 0.0 # activate it with activate_prob
            for rw, prob in zip(self.lrn_id_to_reward_weights[self.learner_id], self.lrn_id_to_reward_weights_activate_prob[
                self.learner_id])
        ]
    ...
    def __str__(self):
        ...
        s_distrib = ('learner_id: {}, zstat_category: {}, '
                     'zeroing_prob: {}').format(self.learner_id, self.zstat_category, self.zeroing_prob)
        s_weight = 'rwd_weights: ' + _str_list(self.reward_weights)
...
class LpLen(Hyperparam):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.total_timesteps = int(self.max_total_timesteps * (1 - (1 - self.minimal_lp_len_ratio) * random.random()))
