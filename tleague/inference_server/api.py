...
class InfServerAPIs(object):
    ...
    def _rebuild_socket(self, socket):
        ...
        if socket in self._dead_sockets:
            ...
            print(f'After {self.timeout} ms for request inference ' f'service {endpoint}, restart a socket!')
    ...
    def _keep_dead_sockets(self, socks, data):
        ...
        long_dead_sockets = [
            socket for socket in self._dead_sockets if (time.time() - self._dead_sockets[socket] > 2 * self.timeout / 1000.)
        ]
