...
class PSROGameMgr(GameMgr):
    ...
    def __init__(self, pgn_file=None, log_file='psro.log', epsilon=0.2, self_prob=0.2, verbose=0, **kwargs):
        super(PSROGameMgr, self).__init__(pgn_file, verbose=verbose)
        self.logger = (
            Logger(dir='.', output_formats=[HOF(log_file)]) if log_file else Logger(dir='.', output_formats=[HOF(sys.stdout)]))
    ...
    def get_player(self, current_player):
        ...
        self.logger.log('get_player(%s): %s' % (current_player, self.players[idx]), level=logger.DEBUG)
    ...
    def get_opponent(self, player, dummy_hyperparam):
        ...
        self.logger.log("get_opponent(%s): %s" % (player, self.players[idx]), level=logger.DEBUG)
        return self.players[idx]

    def get_eval_match(self):
        """ Find least pair of match """
        n_player = len(self.players)
        M = (self.finished_match_counter + self.finished_match_counter.transpose() + np.diag([np.inf] * n_player))
        idx = M.argmin()
        i, j = idx // n_player, idx % n_player
        count = self.finished_match_counter[i, j]
        self.logger.log(
            'get_eval_match(): %s, %s with %s matches finished.' % (self.players[i], self.players[j], count),
            level=logger.DEBUG)
...
class RefCountGameMgr(PSROGameMgr):
    ...
    def get_eval_match(self):
        """ Find least pair of match """
        n_player = len(self.players)
        M = self.all_match_counter + self.all_match_counter.transpose() + np.diag([np.inf] * n_player)
        idx = M.argmin()
        i, j = idx // n_player, idx % n_player
        count = self.finished_match_counter[i, j]
        self.logger.log(
            'get_eval_match(): %s, %s with %s matches finished or ongoing.' % (self.players[i], self.players[j], count),
            level=logger.DEBUG)
        return self.players[i], self.players[j]
...
class PBTEloMatchMakingGameMgr(GameMgr):
  ...
  Allow mutation when accumulating sufficient lp (Learning Period)s """
    ...
    class Individual(object):
        ...
        def __str__(self):
            return ("Individual: elo_rating: {}, trained_lps: {}, "
                    "total_trained_lps: {}, is_living: {}").format(self.elo_rating, self.trained_lps, self.total_lps,
                                                                   self.is_living)

    def __init__(self, pgn_file=None, log_file='elo.log', winrate_threshold=0.3, verbose=0, eligible_lps=1, **kwargs):
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(PBTEloMatchMakingGameMgr, self).finish_match(rowp, colp, outcome, info, match_id)
        if outcome is None: # actor task early abortion, ignore it
            self.logger.logp('finish_match: outcome is None, ignore', level=logger.DEBUG + 2, prob=0.05)
            return
    ...
    means 'continue training'. """
        ...
        if not cur_individual.is_eligible():
            self.logger.log(
                'trained_lps: {} < {}'.format(cur_individual.trained_lps, self.eligible_lps), level=logger.DEBUG + 7)
            return None, False

        # choose the candidate
        living_set = {player: individual for player, individual in self._population.items() if individual.is_living}
        ...
        if winrate < self.winrate_threshold: # cur_player too weak
            ...
            self.logger.log(
                'new_player {},'.format(new_player), 'removed_player: {}'.format(removed_player), level=logger.DEBUG + 7)
        ...
        self.logger.log(
            'cur player: {}, candidate player: {}, win-rate: {}, '.format(current_player, candidate_player, winrate),
    ...
    def get_opponent(self, player, hyperparam):
     ...
     the WHOLE population, regardless of being dead or alive! """
        winrates = np.array([
            elo_predict_winrate(self._population[player].elo_rating, self._population[player_oppo].elo_rating)
    ...
    def _log_population(self, level):
        self.logger.log('Report the whole population,', 'size: {}'.format(len(self._population)), level=level)
...
class PBTPSROGameMgr(GameMgr):
  ...
  optimization in parallel. See [1, 2, 3, 4].  """
    ...
    class Individual(object):
        ...
        def __str__(self):
            return ("Individual: trained_lps: {}, "
                    "total_trained_lps: {}, is_living: {}").format(self.trained_lps, self.total_lps, self.is_living)

    def __init__(self, pgn_file=None, log_file='pbt_psro.log', verbose=0, eligible_lps=2, **kwargs):
    ...
    cases, recreate the tasked opponents for the input player p. """
        ...
        self.logger.log('add_player: {},'.format(p), 'parent_player: {},'.format(parent_p), level=logger.DEBUG + 7)
    ...
    def get_player(self, current_player):
     ...
     contracted value); If small, choose a random one from the living set. """
        ...
        self.logger.log('get_player,', 'cur player: {}'.format(current_player), level=logger.DEBUG + 7)
        # TODO: increment_lp() should be done at on_notify_learner_task_end
        self._population[current_player].increment_lp()
        ...
        if not cur_individual.is_eligible():
            self.logger.log(
                'trained_lps: {} < {},'.format(cur_individual.trained_lps, self.eligible_lps),
                'continue training',
                level=logger.DEBUG + 7)
            return None, False
        ...
        self.logger.log('get_player,', 'living set payoff matrix A: ', level=logger.DEBUG + 7)
        ...
        if cur_d_big_enough:
            ...
            is_mutate = True
        self.logger.log('get_player,', 'new_player: {},'.format(new_player), level=logger.DEBUG + 7)
        self._log_population(level=logger.DEBUG + 7)
        return new_player, is_mutate
    ...
    def _make_tasked_opponents(self, player):
        ...
        self.logger.log(str(A), level=logger.DEBUG + 7)
        self.logger.log('make_tasked_opponents,', 'living_players: {}'.format(living_players), level=logger.DEBUG + 7)

        ind = living_players.index(player)
        vec_payoff = A[ind, :] # (N,)
        vec_p = np.array(vec_payoff > 0.0, dtype=np.float32) # (N,)
        self.logger.log(
            'make_tasked_opponents,', 'ind_player: {},'.format(ind), 'vec_p: {}'.format(vec_p.tolist()), level=logger.DEBUG + 7)
        ...
        if np.all(vec_p < EPS):
            ...
            self.logger.log('make_tasked_opponents,', 'payoff vector all zeros', level=logger.DEBUG + 7)
        else:
            if random.random() < PROB_SELECT_SPECIFIC:
                ...
                self.logger.log('make_tasked_opponents,', 'a specific opponent', level=logger.DEBUG + 7)
            else:
                # collect multiple opponents with non-zero payoff value
                p_opponents = [player for sign_payoff, player in zip(vec_p, living_players) if sign_payoff > EPS]
                self.logger.log('make_tasked_opponents,', 'multiple opponents', level=logger.DEBUG + 7)
        ...
        self.logger.log('make_tasked_opponents,', 'opponents: {}'.format(p_opponents), level=logger.DEBUG + 7)
    ...
    def _get_livingset_payoff_matrix(self):
        ...
        assert living_A.shape[0] == living_A.shape[1], 'A shape {}'.format(living_A.shape)
        assert living_A.shape[0] == len(living_players), 'len: {}'.format(len(living_players))
        return living_A, living_players

    def _get_living_players(self):
        return [player for player, individual in self._population.items() if individual.is_living]
...
class ACMatchMakingGameMgr(GameMgr):
    ...
    class Individual(object):
        ...
        def __str__(self):
            _s_role = lambda i: ('AGENT' if i == ACMatchMakingGameMgr.Role.AGENT else 'COMPETITOR')
            _s_lrnobj = lambda i: ('IMPROVE' if i == ACMatchMakingGameMgr.LrnObj.IMPROVE else 'EXPLOIT')
            return ("Individual: role: {}, trained_lps: {}, total_trained_lps: {}, "
                    "lrn_obj: {}, prev_competitors: {}, "
                    "prev_avg_competitor_winrate: {}".format(
                        _s_role(self.role), self.trained_lps, self.total_trained_lps, _s_lrnobj(self.lrn_obj),
                        list(self.prev_competitors.keys()), self.prev_avg_competitor_winrate))
    ...
    def add_player(self, p, parent_p=None, **kwargs):
        if p == parent_p:
            ...
            self.logger.log('add_player:', 'the same player and parent_player: {},'.format(p), level=logger.DEBUG + 7)
            return
        ...
        self.logger.log('finished_match_counter: {}'.format(self.finished_match_counter), level=logger.DEBUG)
        self.logger.log('all_match_counter: {}'.format(self.all_match_counter), level=logger.DEBUG)
        self.logger.log('sum_outcome: {}'.format(self.sum_outcome), level=logger.DEBUG)
        self.logger.log('add_player:', 'player: {},'.format(p), 'parent_player: {}'.format(parent_p), level=logger.DEBUG + 7)
    ...
    def _add_to_population_as_agent(self, p, parent_p):
        ...
        individual = ACMatchMakingGameMgr.Individual(role=ACMatchMakingGameMgr.Role.AGENT, total_trained_lps=0)
        ...
        if parent_p is not None and parent_p in self._population:
            ...
            individual.prev_avg_competitor_winrate = (self._population[parent_p].prev_avg_competitor_winrate)
    ...
    def _make_lrn_obj(self, p, parent_p):
        ...
        if parent_p == 'None:init_model' or parent_p is None:
            # random sampling
            lrn_obj = (ACMatchMakingGameMgr.LrnObj.IMPROVE if random.random() < 0.65 else ACMatchMakingGameMgr.LrnObj.EXPLOIT)
        else:
    ...
    def _add_to_population_as_competitor(self, p, parent_p):
        ...
        individual = ACMatchMakingGameMgr.Individual(role=ACMatchMakingGameMgr.Role.COMPETITOR, total_trained_lps=0)
        parent_i = None if parent_p is None else self._population[parent_p]
        self.logger.log(
    ...
    def _inherit_player(self, p, parent_p):
        if parent_p == p or parent_p is None:
            self.logger.log(
                '_inherit_player:', 'skipped,', 'p: {},'.format(p), 'parent_p: {}'.format(parent_p), level=logger.DEBUG + 7)
            return
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(ACMatchMakingGameMgr, self).finish_match(rowp, colp, outcome, info, match_id)
        self.logger.logp(
            'finish_match:',
            'rowp: {}'.format(rowp),
            'colp: {}'.format(colp),
    ...
    def _eligible_to_improve(self, current_player):
        ...
        if prev_competitors:
            ...
            new_winrate = self._collect_winrate_vec(current_player, list(prev_competitors.keys()))
        ...
        competitors = self._collect_players_by_role(ACMatchMakingGameMgr.Role.COMPETITOR)
    ...
    def _get_opponent_for_improve(self, player, hyperparam):
        ...
        if not competitors:
            self.logger.log(
                'get_opponent_for_improve:', 'empty competitors, return self {}'.format(player), level=logger.DEBUG + 2)
            return player
    ...
    def _collect_winrate_vec(self, cur_player, other_players, minimal_matches=10):
        winrate, sum_outcome, match_count = self.get_one_vs_all_info(cur_player, other_players, minimal_matches)
        self.logger.log(
            'winrate: {},'.format(winrate),
    ...
    def _log_population(self, level):
        self.logger.log('Report the whole population,', 'size: {}'.format(len(self._population)), level=level)
        for p, i in self._population.items():
            self.logger.log('player: {},'.format(p), 'individual: {}'.format(i), level=level)
...
class ACFixedObjRatioMatchMakingGameMgr(ACMatchMakingGameMgr):
  ...
  Ratio. See [1, 6].  """
    ...
    def __init__(self, pgn_file=None, log_file='acfor.log', verbose=0, **kwargs):
        super(ACFixedObjRatioMatchMakingGameMgr, self).__init__(pgn_file, log_file, verbose)
    ...
    def _make_lrn_obj(self, p, parent_p):
        ...
        self.logger.log('_make_lrn_obj:', 'n_improve: {}, n_exploit: {}'.format(n_improve, n_exploit), level=logger.DEBUG + 7)
        return lrn_obj

    def _compute_lrn_obj_count(self):
        loc = {ACMatchMakingGameMgr.LrnObj.EXPLOIT: 0, ACMatchMakingGameMgr.LrnObj.IMPROVE: 0}
...
class PBTACMatchMakingGameMgr(GameMgr):
    ...
    def __init__(self, pgn_file=None, log_file='ac.log', verbose=0, eligible_lps_by_lobj=None, **kwargs):
    ...
    def _add_to_population_regular(self, p, parent_p):
        ...
        if parent_p is not None:
            assert parent_p in self._population, "{} not in population {}".format(parent_p, self._population)
            self._population[parent_p].role = PBTACMatchMakingGameMgr.Role.COMPETITOR

        # add p to Agent set. In our impl, just add p to population and mark it as
        # Agent. Meanwhile, assign it a learning objective (by random sampling)
        lrn_obj = (PBTACMatchMakingGameMgr.LrnObj.IMPROVE if random.random() < 0.65 else PBTACMatchMakingGameMgr.LrnObj.EXPLOIT)

        individual = PBTACMatchMakingGameMgr.Individual(
            role=PBTACMatchMakingGameMgr.Role.AGENT, total_trained_lps=0, lrn_obj=lrn_obj)
        ...
        self.logger.log('_add_to_population_as_agent:', 'individual: {},'.format(individual), level=logger.DEBUG + 7)
        self._population[p] = individual
    ...
    def _add_to_population_as_competitor(self, p, parent_p):
        ...
        self.logger.log('_add_to_population_as_competitor:', 'individual: {},'.format(individual), level=logger.DEBUG + 7)
        self._population[p] = individual
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(PBTACMatchMakingGameMgr, self).finish_match(rowp, colp, outcome, info, match_id)
        self.logger.logp(
            'finish_match:',
            'rowp: {}'.format(rowp),
            'colp: {}'.format(colp),
    ...
    def _eligible_to_improve(self, current_player):
        is_eligible = True
        if self._population[current_player].trained_lps < self.eligible_lps_by_lobj[PBTACMatchMakingGameMgr.LrnObj.IMPROVE - 1]:
            is_eligible = False
            return is_eligible
        ...
        competitors = self._collect_players_by_role(PBTACMatchMakingGameMgr.Role.COMPETITOR)
    ...
    def _eligible_to_exploit(self, current_player):
        agents = self._collect_players_by_role(PBTACMatchMakingGameMgr.Role.AGENT)
        is_eligible = True
        if self._population[current_player].trained_lps < self.eligible_lps_by_lobj[PBTACMatchMakingGameMgr.LrnObj.EXPLOIT - 1]:
            is_eligible = False
            return is_eligible
