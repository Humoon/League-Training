...
class ModelPoolAPIs(object):
    ...
    def request(self, req):
        ...
        while True:
            try:
                ...
                logger.error("ModelPoolAPIs may crushed on request {}," " the exception:\n{}".format(req, e))
                raise e
    ...
    def push_model(self, model, hyperparam, key, createtime=None, freezetime=None, updatetime=None, learner_meta=None):
        self._pub_lock.acquire()
        self._pub_socket.send_string('model', zmq.SNDMORE)
        self._pub_socket.send_pyobj(Model(model, hyperparam, key, createtime, freezetime, updatetime))
