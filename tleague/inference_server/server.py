...
class InferDataServer(object):
    ...
    def __init__(self, port, batch_size, ds, batch_worker_num=4, use_gpu=True, compress=False):
        ...
        self._rep_server_process = Process(target=rep_server, args=(port, self.rep_ep, self.req_ep), daemon=True)
        ...
        dataset = tf.data.Dataset.range(batch_worker_num).apply(
            tf.contrib.data.parallel_interleave(
                lambda x: tf.data.Dataset.from_generator(self.data_generator, dtypes, shapes),
                cycle_length=batch_worker_num,
                sloppy=True,
                buffer_output_elements=1)).apply(tf.contrib.data.batch_and_drop_remainder(batch_size))
        if use_gpu:
            prefetch_op = tf.contrib.data.prefetch_to_device(device="/gpu:0", buffer_size=1)
...
class InfServer(object):
    ...
    def __init__(self,
        ...
        self.net_out = policy.net_build_fun(self.data_server._batch_input, self.nc, scope='Inf_server')
        # saving/loading ops
        self.params = self.net_out.vars.all_vars
        self.params_ph = [tf.placeholder(p.dtype, shape=p.get_shape()) for p in self.params]
        self.params_assign_ops = [p.assign(np_p) for p, np_p in zip(self.params, self.params_ph)]
    ...
    def load_model(self, loaded_params):
        self._sess.run(
            self.params_assign_ops[:len(loaded_params)], feed_dict={p: v for p, v in zip(self.params_ph, loaded_params)})

    def setup_fetches(self, outputs):

        def split_batch(template, tf_structure):
            split_flatten = zip(*[tf.split(t, self.batch_size) for t in nest.flatten_up_to(template, tf_structure)])
            return [nest.pack_sequence_as(template, flatten) for flatten in split_flatten]

        if self.nc.use_self_fed_heads:
            a = nest.map_structure_up_to(self._ac_structure, lambda head: head.sam, self.net_out.self_fed_heads)
            neglogp = nest.map_structure_up_to(self._ac_structure, lambda head: head.neglogp, self.net_out.self_fed_heads)
            flatparam = nest.map_structure_up_to(self._ac_structure, lambda head: head.flatparam, self.net_out.self_fed_heads)
            self.all_outputs = {
                'a':
                ...
                'v':
                    tf.split(self.net_out.value_head, self.batch_size) if self.net_out.value_head is not None else [[]] *
                    self.batch_size,
                'state':
                    tf.split(self.net_out.S, self.batch_size) if self.net_out.S is not None else [[]] * self.batch_size
            }
        else:
            flatparam = nest.map_structure_up_to(self._ac_structure, lambda head: head.flatparam, self.net_out.outer_fed_heads)
            self.all_outputs = {
                'flatparam': split_batch(self._ac_structure, flatparam),
                'state': tf.split(self.net_out.S, self.batch_size) if self.net_out.S is not None else [[]] * self.batch_size
            }
        if self.nc.use_lstm and 'state' not in outputs:
            outputs.append('state')
        self.fetches = [dict(zip(outputs, pred)) for pred in zip(*[self.all_outputs[o] for o in outputs])]
    ...
    def run(self):
        while not self.data_server.ready:
            time.sleep(10)
            print('Waiting at least {} actors to ' 'connect ...'.format(self.batch_size), flush=True)
        ...
        while True:
            ...
            if t0 > last_log_time + self._log_seconds:
                ...
                print(
                    'Process {} predicts {} samples costs {} seconds, fps {}'.format(pid, sam_num, cost, sam_num / cost),
                    flush=True)
