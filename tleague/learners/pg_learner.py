...
class PGLearner(BaseLearner):
    ...
    def __init__(self,
                 ...
                 ep_loss_coef=None,
                 **kwargs):
        super(PGLearner, self).__init__(league_mgr_addr, model_pool_addrs, learner_ports, learner_id)
        ...
        self._init_const(total_timesteps, burn_in_timesteps, batch_size, unroll_length, rwd_shape, ent_coef, vf_coef,
                         pub_interval, log_interval, save_interval, policy, distill_coef, policy_config, rollout_length,
                         max_grad_norm)
        ...
        # Prepare dataset
        ds = data_type(ob_space, ac_space, self.n_v, use_lstm=self.rnn, hs_len=self.hs_len, distillation=self.distillation)
        ...
        self.trainer = tf.train.AdamOptimizer(learning_rate=self.LR, beta1=adam_beta1, beta2=adam_beta2, epsilon=adam_eps)
        self.burn_in_trainer = tf.train.AdamOptimizer(learning_rate=self.LR, epsilon=1e-5) # same as default and IL
        ...
        if self.use_hvd:
            self.trainer = hvd.DistributedOptimizer(self.trainer, sparse_as_dense=use_sparse_as_dense)
            self.burn_in_trainer = hvd.DistributedOptimizer(self.burn_in_trainer, sparse_as_dense=use_sparse_as_dense)
        ...
        self.broadcast()
        # logging stuff
        format_strs = (['stdout', 'log', 'tensorboard', 'csv'] if self.rank == 0 else ['stdout', 'log', 'tensorboard', 'csv'])
        logger.configure(dir='training_log/{}rank{}'.format(self._learner_id, self.rank), format_strs=format_strs)

    def _build_train_op(self):
        grads_and_vars = self.trainer.compute_gradients(self.loss, self.params)
        grads_and_vars_vf = self.burn_in_trainer.compute_gradients(self.vf_loss, self.params_vf)
    ...
    def run(self):
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                logger.log('rank{}: done init_task and notify_task_begin...'.format(self.rank))
            else:
    ...
    def _init_const(self, total_timesteps, burn_in_timesteps, batch_size, unroll_length, rwd_shape, ent_coef, vf_coef,
                    pub_interval, log_interval, save_interval, policy, distill_coef, policy_config, rollout_length,
                    max_grad_norm):
    ...
    def _build_ops(self):
        ...
        self.reset_optimizer_op = tf.variables_initializer(self.trainer.variables() + self.burn_in_trainer.variables())

        self.loss_names = (list(self.loss_endpoints_names) + ['clip_grad_norm', 'nonclip_grad_norm', 'param_norm'])
        ...
        def _prepare_td_map(lr, cliprange, lam, weights):
            ...
            if weights is None:
                ...
                td_map = {self.LR: lr, self.CLIPRANGE: cliprange, self.rwd_weights: weights}
        ...
        def train_batch(lr, cliprange, lam, weights=None):
            ...
            return self.sess.run(
                self.losses + [self.clip_grad_norm, self.nonclip_grad_norm, self.param_norm, self._train_batch],
                feed_dict=td_map)[0:len(self.loss_names)]

        def burn_in(lr, cliprange, lam, weights=None):
            td_map = _prepare_td_map(lr, cliprange, lam, weights)
            return self.sess.run(
                self.losses + [self.clip_grad_norm_vf, self.nonclip_grad_norm_vf, self.param_norm, self._burn_in],
                feed_dict=td_map)[0:len(self.loss_names)]
        ...
        def load_model(loaded_params):
            ...
            self.sess.run(param_assign_ops, feed_dict={p: v for p, v in zip(self.new_params, loaded_params)})

        def restore_optimizer(loaded_opt_params):
            self.sess.run(self.opt_param_assign_ops, feed_dict={p: v for p, v in zip(self.new_opt_params, loaded_opt_params)})
        ...
        for update in xrange(1, nupdates + 1):
            ...
            if update % self.pub_interval == 0 and self.should_push_model:
                self._model_pool_apis.push_model(
                    self.read_params(), self.task.hyperparam, self.model_key, learner_meta=self.read_opt_params())
            # logging stuff
            if update % self.log_interval == 0 or update == 1:
                lossvals = np.mean(mblossvals, axis=0)
                mblossvals = []
                tnow = time.time()
                consuming_fps = int(nbatch * min(update, self.log_interval) / (tnow - tstart))
                ...
                logger.logkvs({
                    ...
                    scope + "aband_samples": (self._data_server.aband_unroll_num * self.unroll_length)
                })
                logger.logkvs({scope + lossname: lossval for lossname, lossval in zip(self.loss_names, lossvals)})
                logger.dumpkvs()
    ...
    def _init_task(self):
        task = self.task
        logger.log('Period: {},'.format(self._lrn_period_count), 'Task: {}'.format(str(task)))
        ...
        if task.parent_model_key is None:
            logger.log('Parent model is None, ' 'pushing new model {} params to ModelPool'.format(self.model_key))
            self._model_pool_apis.push_model(
                self.read_params(), hyperparam, self.model_key, learner_meta=self.read_opt_params())
        elif self.model_key != self.last_model_key:
            logger.log('Parent model {} exists, pulling its params from ModelPool '
                       'as new model {}'.format(task.parent_model_key, self.model_key))
            model_obj = self._model_pool_apis.pull_model(task.parent_model_key)
            self._model_pool_apis.push_model(model_obj.model, hyperparam, self.model_key, learner_meta=self.read_opt_params())
            ...
            if learner_meta is not None:
                logger.log('Restore optimizer from model {}'.format(task.parent_model_key))
    ...
    def clip_grads_vars(grads_and_vars, all_clip_vars, max_grad_norm):
        nonclip_grads_and_vars = [gv for gv in grads_and_vars if gv[1] not in all_clip_vars]
        ...
        if len(clip_grads_and_vars) > 0:
            clip_grads, clip_vars = zip(*[gv for gv in grads_and_vars if gv[1] in all_clip_vars])
        else:
            clip_grads, clip_vars = [], []
        if max_grad_norm is not None:
            clip_grads, clip_grad_norm = tf.clip_by_global_norm(clip_grads, max_grad_norm)
    ...
    def build_loss(self, model, input_data):
        ...
        if isinstance(self.ent_coef, list):
            ...
            entropy = tf.reduce_sum([e * ec for e, ec in zip(entropy_list, self.ent_coef)])
        else:
        ...
        if self.distillation:
            ...
            if isinstance(self.distill_coef, list):
                ...
                distill_loss = tf.reduce_sum([d * dc for d, dc in zip(distill_losses, self.distill_coef)])
            else:
        ...
        if isinstance(self.vf_coef, list):
            ...
            self.vf_loss = tf.reduce_sum(model.loss.value_loss * tf.constant(self.vf_coef))
        else:
