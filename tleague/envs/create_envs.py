  ...
  game core (StarCraftII, Pommerman, ViZDoom,... ) is loaded only when used, and
   you don't have to install the game core when not used. """
    if arena_id in RECOGNIZED_SC2_ID:
        ...
        return create_sc2_env(arena_id, env_config=env_config, inter_config=inter_config)
    elif arena_id in RECOGNIZED_POMMERMAN_ID:
        from tleague.envs.pomme import create_pommerman_env
        return create_pommerman_env(arena_id, env_config=env_config, inter_config=inter_config)
...
def sc2_test(_):
    ...
    env = create_env(arena_id='sc2full_formal8_dict', env_config=env_config, inter_config=interface_config)
    import numpy as np
    for i in range(2):
        obs = env.reset()
        print(f'episode: {i}')
        start_locations = [ob.game_info.start_raw.start_locations[0] for ob in env.unwrapped._obs]
        base_pos = [
            u.pos for ob in env.unwrapped._obs for u in ob.observation.raw_data.units if u.alliance == 1 and u.unit_type == 86
        ]
        print('---------------')
