...
class Individual(object):
    ...
    def __str__(self):
        return ('sampled_as_oppo_count: {}, total_trained_lps: {},'
                'is_historical: {}').format(self.sampled_as_oppo_count, self.total_trained_lps, self.is_historical)
...
class IndivMainAgent(Individual):
    ...
    def __init__(self, total_trained_lps, main_agent_pfsp_prob, main_agent_forgotten_prob, main_agent_forgotten_me_winrate_thre,
                 main_agent_forgotten_ma_winrate_thre, **kwargs):
    ...
    def decide_descendent(self, individuals, players, winrates, cur_player=None, coin_toss_reset=None):
    ...
    def _pfsp_opponent(self, individuals, winrates):
        ...
        if historical_winrates:
            return np.random.choice(historical_individuals, p=winrates_to_prob(historical_winrates, weighting="squared"))
    ...
    def _forgotten_opponent(self, individuals, winrates):
        ...
        tmp_winrates, tmp_individuals = _remove_monotonic_suffix(historical_winrates, historical_individuals)
...
class IndivSKMainAgent(IndivMainAgent):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(IndivSKMainAgent,
              self).__init__(total_trained_lps, main_agent_pfsp_prob, main_agent_forgotten_prob,
                             main_agent_forgotten_me_winrate_thre, main_agent_forgotten_ma_winrate_thre, **kwargs)
        self.add_to_league_winrate_thre = main_agent_add_to_league_winrate_thre

    def decide_descendent(self, individuals, players, winrates, cur_player=None, coin_toss_reset=None):
        # has sufficiently exploited every historical player?
        historical_winrates = [w for i, w in zip(individuals, winrates) if i.is_historical]
...
class IndivMainExploiter(Exploiter):
    ...
    def __init__(self, total_trained_lps, main_exploiter_min_lps, main_exploiter_max_lps, main_exploiter_winrate_thre,
                 main_exploiter_reset_winrate_thre, main_exploiter_prob_thre, **kwargs):
    ...
    def decide_descendent(self, individuals, players, winrates, cur_player=None, coin_toss_reset=None):
        ...
        active_individuals, active_winrates = zip(
            *[(i, w) for i, w in zip(individuals, winrates) if not i.is_historical and isinstance(i, IndivMainAgent)])
    ...
    def decide_opponent(self, individuals, winrates, coin_toss=None):
        ...
        historical_individuals, historical_winrates = zip(
            *[(i, w) for i, w in zip(individuals, winrates) if i.is_historical and isinstance(i, IndivMainAgent)])
        if historical_winrates:
            return np.random.choice(historical_individuals, p=winrates_to_prob(historical_winrates, weighting="variance"))
...
class IndivSpecExploiter(IndivMainExploiter):
    ...
    def __init__(self, total_trained_lps, spec_exploiter_min_lps, spec_exploiter_max_lps, spec_exploiter_winrate_thre,
                 spec_exploiter_reset_winrate_thre, spec_exploiter_prob_thre, **kwargs):
...
class IndivEvoExploiter(IndivMainExploiter):
  ...
  ml, hl, etc."""
    ...
    def __init__(self, total_trained_lps, evo_exploiter_min_lps, evo_exploiter_max_lps, evo_exploiter_winrate_thre,
                 evo_exploiter_reset_winrate_thre, evo_exploiter_prob_thre, evo_exploiter_pattern, **kwargs):
    ...
    def decide_descendent(self, individuals, players, winrates, cur_player=None, coin_toss_reset=None, active_ma_winrates=None):
        """
...
class IndivAdaEvoExploiter(IndivMainExploiter):
  ...
  same pattern."""
    ...
    def __init__(self, total_trained_lps, ada_evo_exploiter_min_lps, ada_evo_exploiter_max_lps, ada_evo_exploiter_winrate_thre,
                 ada_evo_exploiter_reset_winrate_thre, ada_evo_exploiter_prob_thre, ada_evo_exploiter_pattern, **kwargs):
    ...
    """
        ...
        if self.total_trained_lps >= self.max_lps:
            ...
            descendent = self.find_the_right_leaf_by_pattern(individuals, players, active_ma_winrates)
...
class IndivLeagueExploiter(Exploiter):
    ...
    def __init__(self, total_trained_lps, league_exploiter_min_lps, league_exploiter_max_lps, league_exploiter_winrate_thre,
                 league_exploiter_prob_thre, **kwargs):
    ...
    def decide_descendent(self, individuals, players, winrates, cur_player=None, coin_toss_reset=None):
        ...
        if (self.total_trained_lps >= self.max_lps and cur_coin_toss_reset < self.prob_thre):
    ...
    def decide_opponent(self, individuals, winrates, coin_toss=None):
        ...
        for i, w in zip(individuals, winrates):
            if i.is_historical:
                ...
                historical_winrates.append(w)
        if historical_winrates:
            return np.random.choice(historical_individuals, p=winrates_to_prob(historical_winrates, weighting="linear_capped"))
...
class IndivCyclicExploiter(Exploiter):
    ...
    def __init__(self, total_trained_lps, cyclic_exploiter_n_leaves, cyclic_exploiter_prob_thre, **kwargs):
    ...
    def _find_next_leaf(self, current_player, individuals, players):
        ...
        cyclic_leaves = [leaf for leaf in leaves if isinstance(leaf, IndivCyclicExploiter)]
        ...
        historical_cyclic_leaves = [leaf for leaf in cyclic_leaves if leaf.is_historical]
...
class AEMatchMakingGameMgr(GameMgr):
  ...
  """
    ...
    def __init__(self,
                 ...
                 verbose=0,
                 **kwargs):
        super(AEMatchMakingGameMgr, self).__init__(pgn_file, stat_decay=stat_decay, verbose=verbose)
        self.logger = (
    ...
    """.format(learner_id)
        ...
        if learner_id is not None:
            assert learner_id in self._lrn_id_list, 'unknown learner_id: {}'.format(learner_id)
    ...
    def _add_player_individual(self, p, parent_p, role_cls, pattern=None):
        ...
        assert role_cls in [
            IndivMainAgent, IndivMainExploiter, IndivLeagueExploiter, IndivCyclicExploiter, IndivSKMainAgent,
            IndivSpecExploiter, IndivEvoExploiter, IndivAdaEvoExploiter
        ]
        ...
        if role_cls == IndivEvoExploiter:
            ...
            individual = role_cls(total_trained_lps=0 if parent_i is None else parent_i.total_trained_lps, **self._indiv_kwargs)
    ...
    def _log_add_player(self, p, parent_p, learner_id):
        ...
        self.logger.log(
            ...
            'learner_id: {}'.format(learner_id),
            level=logger.DEBUG + 7)
        self.logger.log('finished_match_counter:\n{}'.format(self.finished_match_counter), level=logger.DEBUG)
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(AEMatchMakingGameMgr, self).finish_match(rowp, colp, outcome, info, match_id)
        self.logger.logp(
            'finish_match,',
            'rowp: {},'.format(rowp),
            'colp: {},'.format(colp),
    ...
    def get_player(self, current_player):
        ...
        if indiv.is_historical:
            self.logger.log(f'get_player for historical player {current_player}!', level=logger.WARN)
        indiv.increment_lp()
        ...
        winrates, _, _ = self.get_one_vs_all_info(cur_player=current_player, other_players=players, minimal_matches=5)

        if isinstance(indiv, IndivEvoExploiter) or \
           isinstance(indiv, IndivAdaEvoExploiter):
            active_individuals, active_players = zip(
                *[(i, p) for i, p in zip(individuals, players) if not i.is_historical and isinstance(i, IndivMainAgent)])
            assert len(active_individuals) == 1, 'only one active Main Agent'
    ...
    def get_opponent(self, player, hyperparam):
        ...
        if indiv.is_historical:
            self.logger.log(f'get_opponent for historical player {player}!', level=logger.WARN)

        players = list(self._population.keys())
        individuals = list(self._population.values())
        winrates, _, _ = self.get_one_vs_all_info(cur_player=player, other_players=players, minimal_matches=5)
        try:
            oppo_indiv = indiv.decide_opponent(individuals, winrates, self._dbg_coin_toss_oppo)
        except:
