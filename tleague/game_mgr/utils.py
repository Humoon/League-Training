...
def make_payoff_matrix(mat_sum_outcome, mat_match_count):
  ...
  """
    A = ((mat_sum_outcome - mat_sum_outcome.transpose()) / (mat_match_count + mat_match_count.transpose() + 1e-8))
    return A
...
def mat2nfg(A, game_title='Title', players=['Player 1', 'Player 2']):
    ...
    descrp = '{ "%s" "%s" } { %d %d }' % (players[0], players[1], A.shape[0], A.shape[1])
    body = ''
...
def _get_gambit_gnm_path():
    ...
    try:
        ...
        print("error while deciding gambit-gnm path {}:{} ".format(str(type(e)), str(e)))
...
def gnm_solve(A):
    ...
    try:
        ...
        res = np.array([float(each) for each in res.decode('utf-8').strip().split(',')[1:]], dtype=np.float32)
        rv = res[0:A.shape[0]]
        cv = res[(A.shape[0]):(A.shape[0] + A.shape[1])]
        pmax = np.max(np.matmul(A, np.reshape(cv, (A.shape[1], 1))))
        pmin = np.min(np.matmul(np.reshape(rv, (1, A.shape[0])), A))
