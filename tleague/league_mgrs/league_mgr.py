...
class LeagueMgr(BaseLeagueMgr):
    ...
    def __init__(self,
        ...
        game_mgr_config['pgn_file'] = (game_mgr_config.get('pgn_file', None) or 'example.pgn')
        ...
        logger.log('__init__: hyperparam_mgr: {}, hyperparam_config: {}'.format(mutable_hyperparam_type,
                                                                                hyperparam_config_name))
        self._hyper_mgr = HyperparamMgr(self._model_pool_apis, mutable_hyperparam_type, hyperparam_config_name)
            ...
            for idx, key_path in enumerate(init_model_paths):
                ...
                with open(model_path, 'rb') as f:
                    ...
                    if 'lrn_id_list' in game_mgr_config:
                        hyperparam = self._hyper_mgr._default_hyperparam(learner_id=game_mgr_config['lrn_id_list'][idx])
                        logger.log('__init__: init model {} has been bound with ' 'hyperparam {}'.format(key, hyperparam))
    ...
    def _on_request_learner_task(self, learner_id):
        ...
        if learner_id not in self._learner_task_table.keys():
            ...
            hyperparam = self._hyper_mgr.get_learner_hyperparam(learner_id, copy_from_model_key=None)
            cur_model_key = None
            ...
            if parent_model_key is None:
                ...
                logger.log("_on_request_learner_task:", "continue training with current model key,", "do Not mutate")
            else:
                # continue training with a new model key or new model key
                model_key = self._gen_new_model_key(old_model_key=parent_model_key)
                logger.log("_on_request_learner_task:", "new model key", "mutate {}: ".format(is_mutate))
        ...
        self.game_mgr.add_player(task.model_key, parent_p=task.parent_model_key, learner_id=learner_id)
        logger.log("_on_request_learner_task: ",
                   "done adding player {}, parent_player {}, to game mgr".format(task.model_key, task.parent_model_key))
    ...
    def _on_request_actor_task(self, actor_id, learner_id):
        ...
        if not isinstance(actor_task, LeagueMgrErroMsg):
            ...
            self.game_mgr.start_match(actor_task.model_key1, actor_task.model_key2, actor_id)

        logger.log("_on_request_actor_task: %s" % str(actor_task), level=logger.DEBUG)
        return actor_task

    def _on_request_eval_actor_task(self, actor_id):
        logger.log("get_eval_actor_task: actor_id:{}".format(str(actor_id)), level=logger.DEBUG)
    ...
    def _on_request_train_actor_task(self, actor_id, learner_id):
        logger.log("_on_request_train_actor_task: actor_id:{}, learner_id:{}".format(actor_id, learner_id), level=logger.DEBUG)
        if (learner_id in self._learner_task_table and self._learner_task_table[learner_id].model_key is not None):
            ...
            if learner_id not in self._learner_task_table:
                logger.log("learner_id({}) hasn't request_learner_task.".format(learner_id), level=logger.WARN)
            elif self._learner_task_table[learner_id].model_key is None:
                logger.log("learner_id({}) hasn't notify_learner_task_begin.".format(learner_id), level=logger.WARN)
            return LeagueMgrErroMsg("Actor task not ready.")
    ...
    def _on_request_add_model(self, model):
        self._model_pool_apis.push_model(model.model, model.hyperparam, model.key, model.createtime, model.freezetime,
                                         model.updatetime)
...
class PARLeagueMgr(LeagueMgr):
  ...
  over less physical learners (e.g., 8). """
    ...
    def __init__(self, port, model_pool_addrs, mutable_hyperparam_type, pseudo_learner_num, **kwargs):
        super(PARLeagueMgr, self).__init__(port, model_pool_addrs, mutable_hyperparam_type, **kwargs)
    ...
    def get_pseudo_id(self, learner_id):
        if learner_id not in self.id_map:
            ...
            logger.log("learner {} begins training with pseude learner_id" " {}".format(learner_id, self.cur_pseudo_id))
    ...
    def _on_notify_learner_task_begin(self, learner_id, learner_task):
        pseudo_id = self.get_pseudo_id(learner_id)
        return super(PARLeagueMgr, self)._on_notify_learner_task_begin(pseudo_id, learner_task)
    ...
    def _on_notify_learner_task_end(self, learner_id):
        ...
        logger.log("learner {} switches from pseudo learner_id {} to "
                   "pseudo learner_id {}".format(learner_id, pseudo_id, self.id_map[learner_id]))
