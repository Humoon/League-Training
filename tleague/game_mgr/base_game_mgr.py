...
class GameMgr(object):
    ...
    def load(self, checkpoint_dir):
        ...
        if len(missing_idx) > 0:
            print('Players {} is missing in model pool'.format([players[i] for i in missing_idx]))
        ...
        self.finished_matches = {
            (rp, cp): c for (rp, cp), c in finished_matches.items() if rp in model_keys and cp in model_keys
        }
    ...
    """
        ...
        if n_players == 0:
            ...
            self.finished_match_counter = np.pad(self.finished_match_counter, (0, 1), 'constant')
    ...
    """
        ...
        if self.avg_match_time is None:
            ...
            self.avg_match_time = 0.9 * self.avg_match_time + 0.1 * (time() - start_time)
        # Write to file
        if self.pgn_file:
    ...
    """
        ...
        r_match_count = (self.finished_match_counter[np.ix_(ind_row, ind_cols)].squeeze(axis=0))
        # column vector (N, 1) -> (N,)
        ...
        c_match_count = (self.finished_match_counter[np.ix_(ind_rows, ind_col)].squeeze(axis=1))
