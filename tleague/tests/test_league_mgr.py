...
class TestLeagueManager(unittest.TestCase):
    ...
    def setUp(self):
        self._model_process = Process(target=lambda: ModelPool(ports="11001:11006").run())
        self._model_process2 = Process(target=lambda: ModelPool(ports="11011:11016").run())
        self._league_process = Process(target=lambda: LeagueMgr(
            port="11007",
    ...
    def test_learner_task(self):
        ...
        league_client.notify_learner_task_begin(learner_id=learner_id, learner_task=task)
    ...
    def test_actor_task(self):
        ...
        league_client.notify_learner_task_begin(learner_id=learner_id, learner_task=learner_task)
        ...
        task = league_client.request_actor_task(actor_id=actor_id, learner_id=learner_id)
        self.assertTrue(isinstance(task, ActorTask))
        league_client.notify_actor_task_begin(actor_id=actor_id)
        league_client.notify_actor_task_end(actor_id=actor_id, match_result=MatchResult(task.model_key1, task.model_key2, 1))
    ...
    def test_checkpoint(self):
        ...
        time.sleep(3)
        checkpoints = [filename for filename in os.listdir("./checkpoints") if filename.startswith("checkpoint")]
        self.assertTrue(len(checkpoints) > 0)

        checkpoint_dir = os.path.join("./checkpoints", checkpoints[-1])
        league_process = Process(target=lambda: LeagueMgr(
            port="11008",
