...
class ReplayMem(object):
    ...
    def __init__(self, rm_size, unroll_length, batch_size, rollout_length, decode=False, minimal_sample=None):
...
class RMFromQueue(object):
    ...
    def __init__(self, input_ep, output_ep, maxlen, minlen, rollout_per_unroll, rollout_length, batch_size, decode):
        self._maxlen = maxlen
        self._minlen = minlen
        self._rollout_length = rollout_length
        ...
        self.pull_data_thread = Thread(target=self._pull_data, args=(pull_socket,), daemon=True)
        self.pull_data_thread.start()
        self.sample_rollout_thread = Thread(target=self._sample_rollout, args=(push_socket,), daemon=True)
        ...
        while True:
            ...
            for _ in range(self._rollout_per_batch):
                ...
                if self._decode:
                    ...
                    batch_data.extend([self._decode_sample(d) for d in unroll[j:j + self._rollout_length]])
            push_socket.send_multipart([pickle.dumps(np.array(d)) for d in zip(*batch_data)], copy=False)
...
class ReplayMemMP(ReplayMem):
    ...
    def __init__(self, rm_size, unroll_length, batch_size, rollout_length, decode=False, n_process=1, minimal_sample=None):
        super(ReplayMemMP, self).__init__(rm_size, unroll_length, batch_size, rollout_length, decode, minimal_sample)
    ...
    @staticmethod
    def _run_sub_rm(input, output, maxlen, minlen, rollout_per_unroll, rollout_length, batch_size, decode):
        p = RMFromQueue(input, output, maxlen, minlen, rollout_per_unroll, rollout_length, batch_size, decode)
        p.run()
    ...
    def _start_subprocess(self):
        ...
        self._subps = [
            Process(
                ...
                args=(self._input_ep, output_ep, maxl, minl, self._rollout_per_unroll, self._rollout_length, self._batch_size,
                      self._decode)) for output_ep in self._output_eps
        ]
...
class ImpSampReplayMem(ReplayMem):
    ...
    def __init__(self, rm_size, unroll_length, batch_size, rollout_length, minimal_sample=None):
        super(ImpSampReplayMem, self).__init__(
            rm_size, unroll_length, batch_size, rollout_length, minimal_sample=minimal_sample)
        self._weights_queue = []
